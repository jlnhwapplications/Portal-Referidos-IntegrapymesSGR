...
import { CheckCircle, Close, CloudUpload, Delete, Description, Image, InsertDriveFile, PictureAsPdf, Warning } from '@mui/icons-material';
...
const ModalAdjuntoGarantia = ({ open, handleClose, id }) => {
    const [disabled, setDisabled] = useState(false);
    const [selectedFiles, setSelectedFiles] = useState([]);
    const { token, referido } = useContext(AuthContext);
    const dispatch = useDispatch();
    const cargaDocumentoSelector = useSelector(store => store.garantias.cargaDocumento)
    ...
    React.useEffect(() => {
        if (cargaDocumentoSelector == "EXITO") {
            dispatch(obtenerAdjuntosGarantias(referido?.accountid, token))
            setDisabled(false)
            setSelectedFiles([])
            setUploadProgress({})
            setTimeout(() => {
    ...
    const handleSubmission = () => {
        ...
        dispatch(cargarAdjuntoGarantia(formData, id, token, toast))
    };
...
export default ModalAdjuntoGarantia