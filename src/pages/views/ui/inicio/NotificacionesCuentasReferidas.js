                ...
                {notification.statecode === 0 && (
                    <Stack direction="row" spacing={1.5}>
                        ...
                        <Chip
                            icon={<PersonIcon />}
                            label={
                                <Tooltip title={notification?.regardingobjectid}>
                                    {notification?.regardingobjectid.length > 16 ? notification?.regardingobjectid.substring(0, 16) + "..." : notification?.regardingobjectid}
                                </Tooltip>}
                            variant="outlined"
                        />
                        <LoadingButton
...
const NotificacionesCuentasReferidas = () => {
    const theme = useTheme()
    const dispatch = useDispatch()
    const router = useRouter()
    const { token } = useContext(AuthContext)
    const { user } = useAuth()
    const { notificacionesCuentasReferidas, loading, inactivarNotificacion } = useGetNotificacionesReferidor()
    ...
    const { pendingNotifications, completedNotifications, totalPending } = useMemo(() => {
        if (!notificacionesCuentasReferidas) return { pendingNotifications: [], completedNotifications: [], totalPending: 0 }

        const pending = notificacionesCuentasReferidas.filter(item => item.statecode === 0)
        const completed = notificacionesCuentasReferidas.filter(item => item.statecode === 1)
    ...
    return (
        <Fragment>
            <Tooltip title="Notificaciones de Cuentas" arrow>
                <IconButton
                    color="inherit"
...
export default NotificacionesCuentasReferidas
